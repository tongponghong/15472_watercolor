...
Helpers::Allocation Helpers::allocate(VkMemoryRequirements const &req, VkMemoryPropertyFlags properties, MapFlag map) {
	return allocate(req.size, req.alignment, 
					find_memory_type(req.memoryTypeBits, properties), 
					map);
}
...
void Helpers::transfer_to_image(void const *data, size_t size, AllocatedImage &target) {
	...
	//std::cout << "texels_per_block" << texels_per_block << std::endl;
	...
	{ // put the receiving image in destination-optimal layout
		VkImageMemoryBarrier barrier {
			...
			// no conditions on earlier commands
			.srcAccessMask = 0,
			// transition must complete before any other transfers write data to image
			.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT,
			// transition old layout -> new layout 
			.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED, // throw away old image
			.newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, // whichever layout is best for receiving data
		...
		// we need to put a barrier here to ensure that all cmds before barrier happen before layout transition
		// and all cmds after must happen after transition
		vkCmdPipelineBarrier(
			transfer_command_buffer, // commandBuffer
			VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, // srcStageMask
			VK_PIPELINE_STAGE_TRANSFER_BIT, // dstStageMask
			0, // dependency flags
			0, nullptr, // memory barrier count, point
	...
	{ // transition the image memory to shader-read-only-optimal layout:
		VkImageMemoryBarrier barrier{
			.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
			// wait until all transfer writes are complete, then transition the image and allow fragment shader reads to proceed 
...
void Helpers::transfer_to_cube_image(void const *data, size_t size, AllocatedImage &target) {
	//refsol::Helpers_transfer_to_image(rtg, data, size, &target);
	assert(target.handle != VK_NULL_HANDLE); // target image should be allocated already
	// check data is the right size [new] 
	size_t bytes_per_block = vkuFormatTexelBlockSize(target.format);
	...
	size_t texels_per_block = vkuFormatTexelsPerBlock(target.format);
	std::cout << "texels_per_block" << texels_per_block << std::endl;
	std::cout << "target" << target.extent.width << "," << target.extent.height << std::endl;
	std::cout << "size " << size << std::endl;
	std::cout << "desired amount" << target.extent.width * target.extent.height * bytes_per_block / texels_per_block << std::endl;
	assert(size == target.extent.width * target.extent.height * bytes_per_block / texels_per_block);
	...
	for (uint32_t face = 0; face < 6; ++face) { // copy the source buffer to the image

		VkBufferImageCopy region{
			.bufferOffset = face * size / 6,
			.bufferRowLength = 0, // since all values are packed 
			.bufferImageHeight = 0,
			.imageSubresource{
				.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
				.mipLevel = 0,
	...
	// maybe need to add more for 6 images?
	vkCmdCopyBufferToImage(
		transfer_command_buffer,
		transfer_src.handle,
		target.handle,
