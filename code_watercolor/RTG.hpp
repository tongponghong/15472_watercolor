...
struct RTG {
	...
	struct Configuration {
		...
		// user camera
		std::string user_entered_camera;

		// exposure
		std::string exp_val;

		std::string user_tonemap_type;
	...
	// scene details 
	S72 scene = S72();
	std::string lamb_lookup_tex = "";

	// -----------------------------------------------
	// camera details 
	std::string userEnteredCamera;

	// -----------------------------------------------
	// exposure and lighting
	float exposure = 0.0f;
	// tonemap type
	std::map< std::string, int > map_tonemaps {{"linear"  , 0}, 
	                                           {"reinhard", 1}};
	int tonemap_type = 0;
